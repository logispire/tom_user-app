...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void _callback(bool isSuccess, String? message, String orderID, int? zoneID,
      double amount, double? maximumCodOrderAmount, Map diffStores) async {
    if (diffStores['counter'] == diffStores['length']) {
      if (isSuccess) {
        if (widget.fromCart) {
            ...
            onPressed: orderController.acceptTerms
                ? () async {
                    ...
                    if (!_isCashOnDeliveryActive! &&
                      ...
                      if (widget.storeId == null) {
                        Map<int, List<Cart>> differentStores = {
                          for (var i in _cartList!) i!.item!.storeId!: []
                        };
                        ...
                        for (int index = 0;
                          ...
                          differentStores[cart.item!.storeId!]?.add(Cart(
                        ...
                        // }
                        int length = differentStores.length;
                        int counter = 0;
                        for (var i in differentStores.entries) {
                          counter += 1;
                          await orderController.placeOrder(
                              ...
                              total,
                              maxCodOrderAmount,
                              {'length': length, 'counter': counter});
                        }
                        print(differentStores);
                      } else {
