...
class StoreRepo {
  ...
  Future<List<String>> setOpenedStores(List<Store?> allStores) async {
    List<String> openedStores = [];
    for (Store? store in allStores) {
      Response res = await apiClient.getData(
        '${AppConstants.storeDetailsUri}${store!.id}',
      );
      store = Store.fromJson(res.body);
      if (Get.find<StoreController>()
          .isStoreOpenNow(store.active!, store.schedules)) {
        openedStores.add(store.id.toString());
      }
    }
    await sharedPreferences.setStringList('opened_stores', openedStores);
    return openedStores;
  }
